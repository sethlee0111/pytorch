...
def _replace_dropout_for_eval(m: GraphModule):
    ...
    # Note: Use ignore_literals=False here because the dropout pattern computes a scalar
    # value used in mul. Otherwise, we would just match the mul and ignore all the ops
    # that went into computing the scalar value.
    replace_pattern_with_filters(
        m, match_pattern, replacement_pattern, match_filters=[], ignore_literals=False,
    )
    m.recompile()
